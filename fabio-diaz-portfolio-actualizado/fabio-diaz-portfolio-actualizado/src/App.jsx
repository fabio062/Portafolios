...
/* Esto es un manifiesto/filosofía propia, NO un testimonio de cliente.
   Se muestra como declaración de autor, nunca como reseña — para no
   generar una impresión falsa de prueba social. */
const manifesto = {
  quote:
    "Cada imagen tiene una intención. Mi trabajo es encontrarla, potenciarla y convertirla en un archivo listo para publicar — sin que se note otra mano.",
  author: "Fabio Diaz",
  role: "Retocador digital",
};

/* ============================================================ */
/* DATOS — Antes y Después (pares para el comparador deslizante)  */
/* IMPORTANTE: cada par debe ser LA MISMA foto (raw vs. editada). */
/* Los pares anteriores usaban fotos de stock de personas         */
/* distintas entre "antes" y "después" — un fotógrafo lo detecta  */
/* al instante y rompe la confianza. Se dejó vacío a propósito:   */
/* súbelo solo cuando tengas tus propios pares reales.            */
/* ============================================================ */
const transformationPairs = [
  // { id: 101, title: "Retoque de piel y color", before: "TU_RAW.jpg", after: "TU_EDITADO.jpg" },
];
  ...
  if (transformationPairs.length === 0) {
    return (
      <section id="transformation" className={`${CONTAINER} pb-20 md:pb-28`}>
        ...
        <Reveal>
          <div className="flex flex-col items-center justify-center text-center gap-4 p-12" style={{ border: `1px dashed ${HAIRLINE}` }}>
            <p className="text-sm max-w-sm" style={{ color: MUTED }}>
              Casos de antes/después reales en preparación. Escríbeme y te envío ejemplos directos de raw vs. entrega final.
            </p>
            <a
              href={MAILTO}
              className="btn-primary text-xs uppercase px-6 py-3 rounded-full"
    ...
    <section style={{ backgroundColor: CREAM }}>
      <div className={`${CONTAINER} py-16 md:py-24`}>
        <Reveal className="max-w-3xl mx-auto text-center px-4">
          <p className="text-xs uppercase mb-6" style={{ ...KICKER, color: MUTED }}>Filosofía</p>
          ...
          <p className="text-xs uppercase" style={{ ...KICKER, color: MUTED }}>
            {manifesto.author} — {manifesto.role}
          </p>
        </Reveal>
      </div>
    </section>
...
/* ============================================================ */
/* CONTACTO — brief estructurado en vez de mailto genérico       */
/* ============================================================ */
function ContactSection() {
  const [form, setForm] = useState({ tipo: "", cantidad: "", plazo: "", mensaje: "" });

  const update = (field) => (e) => setForm((f) => ({ ...f, [field]: e.target.value }));

  const mailtoHref = () => {
    const subject = encodeURIComponent(`Nuevo proyecto — ${form.tipo || "Retoque"}`);
    const bodyLines = [
      `Tipo de proyecto: ${form.tipo || "—"}`,
      `Número de fotos: ${form.cantidad || "—"}`,
  ...
  const labelStyle = { ...KICKER, color: MUTED };

  return (
    <section id="contact" className={`${CONTAINER} pb-20 md:pb-28`}>
      <SectionIntro kicker="Hablemos" heading="Cuéntame de tu proyecto.">
        <p className="text-sm mt-3 max-w-md" style={{ color: MUTED }}>
          Completa estos datos y se abrirá tu correo con todo ya redactado — respondo en menos de 24h.
        </p>
      </SectionIntro>
      ...
      <Reveal>
        <div className="grid md:grid-cols-2 gap-6 mb-6">
          <div>
            <label className="block text-xs uppercase mb-2" style={labelStyle}>Tipo de proyecto</label>
            <select
              value={form.tipo}
              onChange={update("tipo")}
              className="w-full px-4 py-3 text-sm rounded-sm focus:outline-none"
              style={inputStyle}
            >
              <option value="">Selecciona…</option>
              <option value="Sesión individual / retrato">Sesión individual / retrato</option>
              <option value="Boda">Boda</option>
              <option value="Quinceañera">Quinceañera</option>
              <option value="Editorial / campaña">Editorial / campaña</option>
              <option value="Lote grande para estudio">Lote grande para estudio</option>
              <option value="Otro">Otro</option>
            </select>
          </div>

          <div>
            <label className="block text-xs uppercase mb-2" style={labelStyle}>Número aproximado de fotos</label>
            <input
              type="text"
              value={form.cantidad}
              onChange={update("cantidad")}
              placeholder="Ej. 40, 200, 500+"
              className="w-full px-4 py-3 text-sm rounded-sm focus:outline-none"
              style={inputStyle}
            />
          </div>
          ...
          <div>
            <label className="block text-xs uppercase mb-2" style={labelStyle}>Plazo deseado</label>
            <input
              type="text"
              value={form.plazo}
              onChange={update("plazo")}
              placeholder="Ej. esta semana, sin urgencia"
              className="w-full px-4 py-3 text-sm rounded-sm focus:outline-none"
              style={inputStyle}
            />
          </div>
          ...
          <div>
            <label className="block text-xs uppercase mb-2" style={labelStyle}>Mensaje</label>
            <input
              type="text"
              value={form.mensaje}
              onChange={update("mensaje")}
              placeholder="Estilo, referencias, lo que necesites contarme"
              className="w-full px-4 py-3 text-sm rounded-sm focus:outline-none"
              style={inputStyle}
            />
          </div>
